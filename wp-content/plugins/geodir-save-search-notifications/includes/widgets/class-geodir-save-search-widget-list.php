...
/**
 * GeoDir_Save_Search_Widget_List class.
 */
class GeoDir_Save_Search_Widget_List extends WP_Super_Duper {
	...
	public function __construct() {
		...
		$options = array(
			'base_id'        => 'gd_save_search_list',
			'name'           => __( 'GD > Saved Search List', 'geodir-save-search' ),
			'class_name'     => __CLASS__,
			'textdomain'     => GEODIRECTORY_TEXTDOMAIN,
			'block-icon'     => 'fas fa-list',
			'block-category' => 'geodirectory',
			'block-supports' => array(
				'customClassName' => false,
			),
			'block-keywords' => "['geodir','search','list']",
			'widget_ops'     => array(
				'classname'    => 'geodir-save-search-list-container' . ( geodir_design_style() ? ' bsui' : '' ),
				'description'  => esc_html__( 'Displays a button to users to view their saved search list.', 'geodir-save-search' ),
	...
	public function set_arguments() {
		$arguments = array(
			...
			'output' => array(
				...
				'desc' => __( 'Select saved search list button output type.', 'geodir-save-search' ),
				'options' => array(
			...
			'btn_text' => array(
				...
				'desc' => __( 'Saved search list button title.', 'geodir-save-search' ),
				'placeholder' => __( 'View Saved Search', 'geodir-save-search' ),
				'default' => '',
				'desc_tip' => true,
				'advanced' => false,
	...
	public function output_html( $instance = array(), $args = array() ) {
		...
		$instance = shortcode_atts( $defaults, $instance, 'gd_save_search_list' );

		$show = is_user_logged_in();
		if ( ! apply_filters( 'geodir_save_search_list_show_output', $show, $instance ) ) {
			return;
		}
		...
		$button_class = 'btn';
		$default_text = __( 'View Saved Search', 'geodir-save-search' );
		...
		$wrap_class = geodir_build_aui_class( $instance ) . ' geodir-saved-search-btn-wrap';
		...
		if ( empty( $instance['is_preview'] ) ) {
			...
			$extra = apply_filters( 'geodir_save_search_list_extra_params', $extra, $extra_params, $instance );

			$onclick = "geodir_save_search_aui_modal('" . esc_attr( addslashes( $default_text ) ) . "','" . esc_attr( addslashes( $extra ) ) . "', 1); return false;";
		} else {
			$onclick = "";
		}
	...
	public static function add_script() {
		...
		GeoDir_Save_Search_Post::get_widget_script( 'gd_save_search_list' );
	}
}

...
class Assets extends Controller {
	...
	private function register_general_scripts() {
		...
		$default_footer_text = sprintf( esc_html__( 'Made with %s by WPMU DEV', 'wds' ), '<span class="sui-icon-heart" aria-hidden="true" aria-label="love"></span>' );
	...
	private function register_instant_indexing_page_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'    => array(
					'empty_url'  => esc_html__( 'The URL field is empty. Please enter at least one URL and try again.', 'wds' ),
					'success'    => esc_html__( 'Submission Completed!', 'wds' ),
					'limit'      => esc_html__( 'You cannot submit more than 100 URLs at once.', 'wds' ),
					'invalid'    => esc_html__( 'One or more URLs are invalid. Please check and try again.', 'wds' ),
					'wrong'      => esc_html__( 'Something went wrong. Please try again.', 'wds' ),
					'rate_limit' => esc_html__( 'Rate limit exceeded. Please wait and try again later.', 'wds' ),
				),
			)
		);
	}

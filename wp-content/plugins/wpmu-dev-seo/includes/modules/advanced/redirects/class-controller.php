...
class Controller extends Controllers\Submodule_Controller {
	...
	public function save_redirect() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( esc_html__( 'You do not have permission to perform this action.', 'wds' ) );
		}
	...
	public function delete_redirect() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array( 'message' => esc_html__( 'You do not have permission to perform this action.', 'wds' ) )
			);
		}
		$data = $this->get_request_data();

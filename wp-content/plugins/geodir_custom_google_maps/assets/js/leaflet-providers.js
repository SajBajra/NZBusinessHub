(function (root, factory) {
	...
	L.TileLayer.Provider = L.TileLayer.extend({
		initialize: function (arg, options) {
			...
			/* Modified to set apiKey*/
			if (typeof provider.options.apiKey != 'undefined' && typeof geodir_params != 'undefined') {
				if (variantName && typeof geodir_params.providersApiKeys == 'object' && geodir_params.providersApiKeys[providerName + '_' + variantName ]) {
					provider.options.apiKey = geodir_params.providersApiKeys[providerName + '_' + variantName ];
				} else if (typeof geodir_params.providersApiKeys == 'object' && geodir_params.providersApiKeys[providerName]) {
					provider.options.apiKey = geodir_params.providersApiKeys[providerName];
				} else if (geodir_params.providerApiKey) {
					provider.options.apiKey = geodir_params.providerApiKey;
				} else if (provider.options.defaultKey) {
					provider.options.apiKey = provider.options.defaultKey;
				}
			}
	...
	L.TileLayer.Provider.providers = {
		...
		MapTilesAPI: {
			...
			options: {
				...
				defaultKey: '91eb180eb9msh46beac27e6084cep17a106jsn18a17417dbb6',
				maxZoom: 19
			},
			variants: {
				OSMEnglish: {
		...
		Thunderforest: {
			...
			options: {
				...
				variant: 'cycle',
				apiKey: '<insert your api key here>',
				defaultKey: 'db5ae1f5778a448ca662554581f283c5',
				maxZoom: 22
			},
			variants: {
				OpenCycleMap: 'cycle',
		...
		Jawg: {
			url: 'https://tile.jawg.io/{variant}/{z}/{x}/{y}{r}.png?access-token={apiKey}',
			options: {
				attribution:
				...
				apiKey: '<insert your access token here>',
				defaultKey: 'PyTJUlEU1OPJwCJlW1k0NC8JIt2CALpyuj7uc066O7XbdZCjWEL3WYJIk6dnXtps'
			},
			variants: {
		...
		MapBox: {
			url: 'https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}{r}?access_token={apiKey}',
			options: {
				attribution:
		...
		MapTiler: {
			url: 'https://api.maptiler.com/maps/{variant}/{z}/{x}/{y}{r}.{ext}?key={apiKey}',
			options: {
				attribution:
					'<a href="https://www.maptiler.com/copyright/" target="_blank">&copy; MapTiler</a> <a href="https://www.openstreetmap.org/copyright" target="_blank">&copy; OpenStreetMap contributors</a>',
				variant: 'streets',
				ext: 'png',
				apiKey: '<insert your MapTiler Cloud API key here>',
				tileSize: 512,
				zoomOffset: -1,
				minZoom: 0,
				maxZoom: 21
		...
		OpenWeatherMap: {
			...
			options: {
				...
				apiKey:'<insert your api key here>',
				defaultKey:'b5e058d8b5e273b73472107c216b08b4',
				opacity: 0.5
			},
			variants: {
				Clouds: 'clouds',
		...
		NLS: {
			...
			// https://maps.nls.uk/projects/subscription-api
		...
		/*ChinaTianDiTu: {
			url: '//t{s}.tianditu.com/DataServer?T={variant}&X={x}&Y={y}&L={z}&tk={key}',
			options: {
			...
			variants: {
				NormalMap: 'vec_w',
				NormalAnnotion: 'cva_w',
				SatelliteMap: 'img_w',
				SatelliteAnnotion: 'cia_w',
				TerrainMap: 'ter_w',
				TerrainAnnotion: 'cta_w'
			}
		},*/
		ChinaGaoDe: {
		...
		ChinaGoogle: {
			url: '//www.google.cn/maps/vt?lyrs={variant}&gl=cn&x={x}&y={y}&z={z}',
			options: {
				attribution: '&copy; <a href="https://www.google.cn/">Google</a>',
				maxZoom: 18,
				minZoom: 5
			},
			variants: {

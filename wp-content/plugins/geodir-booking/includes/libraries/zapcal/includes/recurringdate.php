...
namespace GeoDir\Bookings\Libraries\iCalendar;

use \Exception;

// no direct access
defined('_GEODIR_ICALENDAR') or die( 'Restricted access' );
...
class ZCRecurringDate {
	...
	function getDates($maxdate = null){
		...
		while(!$done){
			...
			if($year > _GEODIR_ICALENDAR_MAXYEAR)
			{
				$done = true;
			}
			$loopcount++;
			if($loopcount > _GEODIR_ICALENDAR_MAXYEAR){
				$done = true;

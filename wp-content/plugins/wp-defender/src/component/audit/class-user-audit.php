...
class User_Audit extends Audit_Event {
	...
	public function get_hooks(): array {
		...
		if ( is_multisite() ) {
			...
			$hooks['revoke_super_admin'] = array(
				'args'         => array( 'user_id' ),
				'text'         => sprintf(
				/* translators: 1: Blog name. 2: Actor username. 3: Affected username. */
	 ...
	 * @return bool|array
	 */
	public function created_new_user_callback( string $hook_name, array $params ): bool|array {
		...
		return array(
			sprintf(
			/* translators: 1: Blog name, 2: Username, 3: User role. */
				esc_html__( 'User created on %1$s, Username: %2$s, Role: %3$s', 'wpdef' ),
				$blog_name,
				$username,
				$role
			),

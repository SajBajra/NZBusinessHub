...
class Scan extends Event {
	...
	public function __construct() {
		...
		// On some installs the Quarantine controller file is missing, but the
		// Composer classmap still references it. Using class_exists() with
		// autoload disabled avoids noisy include() warnings in that case.
		if ( class_exists( 'WP_Defender\Controller\Quarantine', false ) ) {
	...
	public function status(): Response {
		...
		if ( is_object( $scan ) && ! is_wp_error( $scan ) ) {
			...
			$response['message'] = __( 'Malware scan completed successfully!', 'wpdef' );
		...
		if ( $scan instanceof Model_Scan ) {
			...
			if ( is_object( $item ) && $item->has_method( $intention ) ) {
				...
				if ( 'quarantine' === $intention ) {
					$result = $item->$intention( $data['parent_action'], $item->owner );
				} else {

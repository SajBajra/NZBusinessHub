...
ob_start();
?>
<script type="text/javascript">jQuery(document).on('click','.<?php echo trim($identifier);?> .gd-wgt-page',function(e){var obj = this, pid = parseInt(jQuery(this).data('page')), container = jQuery(obj).closest('.gd-rows-popular-locations'), loading = jQuery('.gd-ajax-wgt-loading', jQuery(container).closest('.geodir-widget'));if(!pid > 0 || !(container && typeof container != 'undefined')){return false;}var scatts = "<?php echo addslashes(json_encode($widget_atts));?>";var data = {'action': 'gd_popular_location_list','_nonce': '<?php echo wp_create_nonce("geodir-popular-location-nonce");?>','geodir_ajax': true,'pageno': pid,'scatts': scatts};jQuery(document).ajaxStop(function(){jQuery('ul',container).css({'opacity':'1'});loading.hide();});jQuery('ul', container).css({'opacity':'0.4'});loading.show();jQuery.post(geodir_params.ajax_url, data, function(response){if(response && response != '0'){loading.hide();jQuery(container).html(response);geodir_init_lazy_load();}});});</script>
<?php
$content .= ob_get_clean();
	...
	if ( $aui_bs5 && function_exists( 'aui_bs_convert_sd_output' ) ) {
		$content = aui_bs_convert_sd_output( $content );
	}

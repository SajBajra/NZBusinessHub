<?php
/**
 * Dynamic User Emails Query class
 *
 * @author    AyeCode Ltd
...
/**
 * GeoDir_Dynamic_Emails_List_Query class.
 */
class GeoDir_Dynamic_Emails_List_Query {
	...
	public function __construct( $query = null ) {
		$this->table = GEODIR_DYNAMIC_EMAILS_LISTS_TABLE;
	...
	public function prepare_query( $query = array() ) {
		...
		do_action_ref_array( 'geodir_dynamic_emails_pre_get_email_lists', array( &$this ) );
		...
		if ( is_array( $qv['fields'] ) ) {
			...
			if ( empty( $qv['fields'] ) ) {
				$qv['fields'] = array( 'email_list_id' );
			}

			$this->query_fields = array();
			foreach ( $qv['fields'] as $field ) {
				$field = 'email_list_id' === $field ? 'email_list_id' : sanitize_key( $field );
			...
			$field = 'email_list_id' === strtolower( $qv['fields'] ) ? 'email_list_id' : sanitize_key( $qv['fields'] );
			$this->query_fields = "$this->table.$field";
		}
		...
		// post_type
		if ( '' !== $qv['post_type'] ) {
			if ( ! empty( $qv['post_type__empty'] ) ) {
				$this->query_where .= $wpdb->prepare( " AND ( `post_type` = %s OR `post_type` IS NULL OR `post_type` = '' )", $qv['post_type'] );
			} else {
		...
		if ( ! empty( $qv['post_type__in'] ) ) {
			$post_type__in = ! empty( $qv['post_type__empty'] ) ? array( "`post_type` IS NULL OR `post_type` = ''" ) : array();
		...
		// category
		if ( '' !== $qv['category'] ) {
			if ( ! empty( $qv['category__empty'] ) ) {
				$this->query_where .= $wpdb->prepare( " AND ( `category` = %s OR `category` IS NULL OR `category` = '' )", $qv['category'] );
			} else {
		...
		if ( ! empty( $qv['category__in'] ) ) {
			$category__in = ! empty( $qv['category__empty'] ) ? array( "`category` IS NULL OR `category` = ''" ) : array();
		...
		// Roles.
		if ( $qv['role'] != '' ) {
			if ( ! empty( $qv['role__empty'] ) ) {
				$this->query_where .= $wpdb->prepare( " AND ( `user_roles` = %s OR `user_roles` IS NULL OR `user_roles` = '' )", $qv['role'] );
			} else {
				$this->query_where .= $wpdb->prepare( " AND `user_roles` = %s", $qv['post_type'] );
			}
		}

		if ( ! empty( $qv['role__in'] ) ) {
			$role__in = ! empty( $qv['role__empty'] ) ? array( "`user_roles` IS NULL OR `user_roles` = ''" ) : array();
		...
		if ( empty( $qv['orderby'] ) ) {
			// Default order is by 'email_list_id'.
			$ordersby = array( 'email_list_id' => $order );
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = "`email_list_id` $order";
		}
		...
		if ( $search ) {
			...
			if ( $qv['search_columns'] ) {
				$search_columns = array_intersect( $qv['search_columns'], array( 'email_list_id', 'action', 'name', 'post_type', 'category', 'user_roles', 'subject', 'template', 'meta', 'date_added', 'status' ) );
			}
			if ( ! $search_columns ) {
				$search_columns = array( 'email_list_id', 'action', 'name', 'post_type', 'category', 'user_roles', 'subject', 'template', 'meta', 'date_added', 'status' );
			}
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids                = implode( ',', $include );
			$this->query_where .= " AND $this->table.email_list_id IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids                = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $this->table.email_list_id NOT IN ($ids)";
		}
		...
		$this->query_where = str_replace( "WHERE 1=1 AND ", "WHERE ", $this->query_where );
	...
	protected function parse_orderby( $orderby ) {
		...
		$_orderby = '';
		if ( in_array( $orderby, array( 'email_list_id', 'action', 'name', 'date_added', 'status' ), true ) ) {
			$_orderby = $orderby;
		}

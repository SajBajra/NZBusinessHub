<?php
/**
 * Contains the customer bookings widget class.
 *
 * @since   1.0.0
 */
...
/**
 * GeoDir_Customer_Bookings_Widget class.
 *
 * @since 1.0.0
 */
class GeoDir_Customer_Bookings_Widget extends WP_Super_Duper {
	...
	public function __construct() {
		...
		$options = array(
			...
			'base_id'        => 'gd_customer_bookings',										// this is used as the widget id and the shortcode id.
			'name'           => __( 'GD > All Customer Bookings', 'geodir-booking' ),				    // the name of the widget.
			'widget_ops'     => array(
				'classname'       => 'geodir-customer-bookings-wrapper bsui',                         // widget class
				'description'     => esc_html__( 'Displays all bookings by the current user', 'geodir-booking' ),   // widget description
				'gd_is_booking'   => true,
			),
...
//		$arguments['text_color'] = sd_get_text_color_input(); // does not work with tables very well
		...
		return apply_filters( 'geodir_customer_bookings_widget_arguments', $arguments );
	}

	/**
	 * Outputs the customer's bookings.
	 *
	 * @param array $args
	...
	public function output( $args = array(), $widget_args = array(), $content = '' ) {
		...
		if ( ! empty( $args['id'] ) ) {
			...
			return aui()->alert( array(
				'type'    => 'warning',
				'content' => __( 'Log in to view your bookings.', 'geodir-booking' ),
			) );
		}

		$GLOBALS['geodir_booking_load_all_customer_bookings_script'] = true;
		$wrap_class = sd_build_aui_class( $args );
		return geodir_get_template_html( 'customer-bookings.php', array( 'wrap_class' => $wrap_class, 'user_id' => $user_id, 'is_preview' => $this->is_preview() ), 'geodir-booking', plugin_dir_path( GEODIR_BOOKING_FILE ) . 'templates' );
	}

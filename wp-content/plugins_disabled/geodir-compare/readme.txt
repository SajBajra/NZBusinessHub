=== GeoDirectory Compare Listings ===
Contributors: stiofansisland, paoltaia, ayecode
Donate link: https://wpgeodirectory.com
Tags: compare, compare listing, compare listings, geodirectory
Requires at least: 5.0
Tested up to: 6.8
Requires PHP: 5.6
Stable tag: 2.2.3
License: GPLv3
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Compare listings side by side and compare vital information.

== Description ==

= Compare all the things! =

Use the Compare Listings addon to let your users view listings side by side and compare vital information.

Many types of directory websites can benefit from a quick and easy comparison chart. For example, users of classifieds or accommodation directory can quickly compare things like features and prices of multiple listings. Indeed, many directory types can take advantage of such functionality.

= Compare Listings key features =

Will this be the feature your users can't live without? Many large directories are using such functionality. We have taken the best features from them all to create our addon.

== Installation ==

1. Upload 'geodir-compare' directory to the '/wp-content/plugins/' directory
2. Activate the plugin "GeoDirectory Compare Listings" through the 'Plugins' menu in WordPress
3. Go to WordPress Admin -> GeoDirectory -> Settings -> Pages and set the main listings comparison page
4. (Optional) Go to the widgets page and add the Geodir Compare widget.

== Changelog ==

= 2.2.3 - 2025-08-14 =
* Field titles in compare list are not translatable with WPML - FIXED

= 2.2.2 - 2024-08-27 =
* Don't show field value when field is excluded in package - FIXED

= 2.2.1 - 2024-08-22 =
* Show listings in same order as ids added in url - CHANGED

= 2.2 - 2023-03-16 =
* Changes for AUI Bootstrap 5 compatibility - ADDED

= 2.1.3 (2022-10-05) =
* Option added in compare button to open compare list in a separate page - ADDED

= 2.1.2 (2022-08-09) =
* compare button 2nd click is not working - FIXED

= 2.1.1.0 (2021-10-08) =
* Prevent the block/widget class loading when not required - CHANGED

= 2.1.0.2 (2021-06-08) =
* Compare Listings menu page don't shows listings with AUI - FIXED

= 2.1.0.1 (2020-10-27) =
* Update Font Awesome icons - CHANGED
* FlexSlider is not loading in compare lists popup - FIXED

= 2.1.0.0 (2020-10-08) =
* Changes for AyeCode UI compatibility - CHANGED

= 2.0.0.3 (2020-01-29) =
* Empty listing results compare message messes the page layout - FIXED

= 2.0.0.2 (2019-07-30) =
* Duplicate image slider id breaks ajax image slider - FIXED

= 2.0.0.1 (2019-07-06) =
* First time visitors can't remove an item from the comparison table - FIX
* UX improvements
* Show comparison list directly in the popup

= 2.0.0.0 (2019-07-02) =
* Initial Release